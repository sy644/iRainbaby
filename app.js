...
function render() {
  ...
  // 把当前激活的 tab 滚到 dock 中央 (新增: 横向滑动支持)
  (function scrollActiveTabIntoView() {
    var bar = document.querySelector('.dock-bar');
    if (!bar) return;
    var active = bar.querySelector('.tab.active') || bar.querySelector('.tab-summary.active');
    if (!active) return;
    var barRect = bar.getBoundingClientRect();
    var aRect = active.getBoundingClientRect();
    // 目标: 让 active 中心对齐到 bar 中心
    var delta = (aRect.left + aRect.width / 2) - (barRect.left + barRect.width / 2);
...
function renderFund(f, i) {
  ...
  return `
    <div class="fund" style="border-top: 4px solid ${f.color}">
      ...
      ${(() => {
        // ===== 区间条: 跑道 + 跑步小人 + 树 + 星星 (用户已要求移除, 留空保留函数便于恢复) =====
        return '';
      })()}
